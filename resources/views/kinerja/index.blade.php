...
<div class="blog-container">
  ...
  <div class="blog-body">
    ...
    <div class="blog-title">
      <h6>Data kinerja dari {{$user->user->name}} </h6>
      <h6></h6>
        ...
        <table class="display table table-bordered" id="dataTable" width="100%" cellspacing="0">
          ...
          <thead style="">
            <tr>
              ...
              <th>Output</th>
              <th>Kuantitas Reliasisasi</th>
              <th>Output</th>
              <th>Total perkinerja</th>
              <th>Action</th>
         ...
         @foreach($kinerjas as $key => $items)
                <td>{{ $items->kinerja }} </td>
                <td>{{ $items->kuantitast }} </td>
                <td>{{ $items->outputt }} </td>
                <td>{{ $items->kuantitasr }} </td>
                <td>{{ $items->outputr }} </td>
                <td>{{ $items->total}} % </td>
                <td><a class="btn btn-sm btn-success" style="color:white" type="submit"
                  href="/editkinerja/{{ $items->id }}">Edit</a>
              ...
              @endforeach
              <tr><td colspan="5"><b>TOTAL (Realisasi/Target x 100) </b></td><td><b>{{$totalz}} %</b></tr>
                <tr><td colspan="5"><b>PENGURANGAN (100 - Realisasi/Target x 100) </b></td><td><b>{{100 -$totalz}} %</b></tr>
