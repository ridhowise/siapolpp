      ...
      <div class="table-responsive">
        <table class="display table table-bordered" id="dataTable" width="100%" cellspacing="0">
          ...
          <tbody>
            ...
            $no = 1;
            @endphp
         @foreach($produktifitas as $key => $items)
                <td>{{ $no++ }}</td>
                ...
                <td><a class="btn btn-sm btn-info" style="color:white" type="submit"
                    href="/kinerja/{{ $items->id }}"><i class="fas fa-eye"></i> Lihat Detail</a>
                </td>
                            ...
                            <div class="form-group row"><label class="col-lg-3 form-control-label">Nama </label>
                                ...
                                <div class="col-lg-5">
                                    <select name="nama" id="nama" class="form-control select" required>
                                        ...
                                        <option value="{{$items->user->id}}">{{$items->user->name}}</option>
                                       @endforeach
                                    </select>
                                </div>
                            </div>
  ...
  $(document).ready(function() {
      ...
      $('#add').click(function() {
          ...
          $('#dynamic_field').append('<tr id="row' + i +
              '" class="dynamic-added"><td> <textarea value="" name="kinerja[]" type="text"  class="form-control"></textarea></td><td> <button type="button" name="remove" id="' + i + '" class="btn btn-danger btn_remove">X</button></td></tr><tr><td><input type="text" value="" class="form-control" name="kuantitast[]" placeholder="Kuantitas Target"></td></tr><tr><td><input type="text" value="" class="form-control" name="kuantitasr[]" placeholder="Kuantitas Realisasi"></td></tr><tr><td><input type="text" value="" class="form-control" name="outputt[]" placeholder="Output"></td></tr>');
      });
